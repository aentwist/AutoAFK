...
cwd = os.path.dirname(
    __file__
)  # We prefix all file calls with cwd so we can call from other directories (I.E via batch or cron)
...
parser.add_argument(
    "-at", "--autotower", action="store_true", help="Run the Auto-Towers function"
)
parser.add_argument(
    "-tower",
    "--tower",
    ...
    help="Select and run the given tower",
)
parser.add_argument("-t", "--test", action="store_true", help="Auto-launch Test server")
parser.add_argument(
    "-l", "--logging", action="store_true", help="Log output to text file"
)
...
repo_releases = requests.get(
    "https://api.github.com/repos/Hammanek/AutoAFK/releases/latest"
)
json = (
    repo_releases.json() if repo_releases and repo_releases.status_code == 200 else None
)
if json != None:
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # Dailies button
        self.dailiesButton = customtkinter.CTkButton(
            master=self.dailiesFrame, text="Run Dailies", command=start_dailies_thread
        )
        self.dailiesButton.place(x=20, y=10)

        # Activities button
        self.activitiesButton = customtkinter.CTkButton(
        ...
        # Shop button
        self.dailiesShopButton = customtkinter.CTkButton(
            master=self.dailiesFrame,
            text="Store Options",
            fg_color=["#3B8ED0", "#1F6AA5"],
            width=120,
        ...
        # Advanced button
        self.advancedButton = customtkinter.CTkButton(
            master=self.dailiesFrame,
        ...
        # Activities button
        self.arenaButton = customtkinter.CTkButton(
            master=self.arenaFrame, text="Run Activity", command=start_activity_thread
        )
        self.arenaButton.place(x=20, y=15)

        # Activities Dropdown
        self.activityFormationDropdown = customtkinter.CTkComboBox(
        ...
        # Activities Entry
        self.pvpLabel = customtkinter.CTkLabel(
            master=self.arenaFrame,
            text="How many battles",
            fg_color=("gray86", "gray17"),
        )
        self.pvpLabel.place(x=10, y=90)
        self.pvpEntry = customtkinter.CTkEntry(
            master=self.arenaFrame, height=20, width=40
        )
        ...
        # Push Button
        self.pushButton = customtkinter.CTkButton(
            master=self.pushFrame, text="Auto Push", command=start_push_thread
        )
        self.pushButton.place(x=20, y=10)

        # Push Entry
        self.pushLocationDropdown = customtkinter.CTkComboBox(
            master=self.pushFrame, values=["Campaign"], width=160
        )
        self.pushLocationDropdown.place(x=10, y=50)

        # Push Formation
        self.pushLabel = customtkinter.CTkLabel(
            master=self.pushFrame,
            text="Which formation?",
            fg_color=("gray86", "gray17"),
        )
        self.pushLabel.place(x=10, y=80)
        self.pushFormationDropdown = customtkinter.CTkComboBox(
            master=self.pushFrame, values=["1st", "2nd", "3rd", "4th", "5th"], width=80
        )
        self.pushFormationDropdown.set(config.get("PUSH", "formation"))
        self.pushFormationDropdown.place(x=10, y=110)

        # Push Artifacts y/n
        self.useArtifactsLabel = customtkinter.CTkLabel(
            master=self.pushFrame, text="Copy Artifacts", fg_color=("gray86", "gray17")
        )
        self.useArtifactsLabel.place(x=10, y=145)
        self.useArtifactsCheckbox = customtkinter.CTkCheckBox(
            master=self.pushFrame,
            text=None,
            onvalue=True,
            offvalue=False,
            command=self.updateArtifacts,
        )
        ...
        if (
            latest_release.split(" ")[1] != version
            and latest_release.split(" ")[1] != "retrieve!"
        ):
            if version.replace(".", "") < latest_release.split(" ")[1].replace(".", ""):
                ...
                self.textbox.insert(
                    "end",
                    "Download here: https://github.com/Hammanek/AutoAFK/releases/\n\n",
                    "yellow",
                )
...
class activityWindow(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.label = customtkinter.CTkLabel(
            master=self.activityFrame, text="Activities:", font=("Arial", 15, "bold")
        )
        self.label.place(x=10, y=5)

        # Campaign Screen
        # AFK Rewards Collect
        self.collectRewardsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect AFK Rewards",
            fg_color=("gray86", "gray17"),
        )
        self.collectRewardsLabel.place(x=10, y=40)
        self.collectRewardsCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
            command=self.activityUpdate,
        )
        self.collectRewardsCheckbox.place(x=200, y=40)
        # Mail Collect
        self.collectMailLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect Mail",
            fg_color=("gray86", "gray17"),
        )
        self.collectMailLabel.place(x=10, y=70)
        self.collectMailCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.companionPointsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Send/Receive Companion Points",
            fg_color=("gray86", "gray17"),
        )
        self.companionPointsLabel.place(x=10, y=100)
        self.companionPointsCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Send Mercs
        self.lendMercsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Auto lend mercs?",
            fg_color=("gray86", "gray17"),
        )
        self.lendMercsLabel.place(x=40, y=130)
        self.lendMercsCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Fast Rewards
        self.fastRewardsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Fast Rewards",
            fg_color=("gray86", "gray17"),
        )
        self.fastRewardsLabel.place(x=10, y=160)
        self.fastrewardsEntry = customtkinter.CTkEntry(
            master=self.activityFrame, height=20, width=25
        )
        ...
        self.attemptCampaignLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Attempt Campaign",
            fg_color=("gray86", "gray17"),
        )
        self.attemptCampaignLabel.place(x=10, y=190)
        self.attemptCampaignCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Spooky Forest
        # Fountain of Time
        self.fountainOfTimeLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect Fountain of Time",
            fg_color=("gray86", "gray17"),
        )
        self.fountainOfTimeLabel.place(x=10, y=220)
        self.fountainOfTimeCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Kings Tower
        self.kingsTowerLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Attempt King's Tower",
            fg_color=("gray86", "gray17"),
        )
        self.kingsTowerLabel.place(x=10, y=250)
        self.kingsTowerCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Collect Inn gifts
        self.collectInnLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect Inn Gifts",
            fg_color=("gray86", "gray17"),
        )
        self.collectInnLabel.place(x=10, y=280)
        self.collectInnCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Battle Guild Hunts
        self.guildHuntLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Battle Guild Hunts",
            fg_color=("gray86", "gray17"),
        )
        self.guildHuntLabel.place(x=10, y=280)
        self.guildHuntCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Ranhorn + Lab
        # Store Purchases
        self.storePurchasesLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Make Store Purchases",
            fg_color=("gray86", "gray17"),
        )
        self.storePurchasesLabel.place(x=10, y=310)
        self.storePurchasesCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Shop Refresh Amount
        self.shoprefreshLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Store Refreshes",
            fg_color=("gray86", "gray17"),
        )
        self.shoprefreshLabel.place(x=40, y=340)
        self.shoprefreshEntry = customtkinter.CTkEntry(
            master=self.activityFrame, height=20, width=25
        )
        self.shoprefreshEntry.insert("end", config.get("DAILIES", "shoprefreshes"))
        self.shoprefreshEntry.place(x=200, y=340)
        # Twisted Realm
        self.twistedRealmLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Attempt Twisted Realm",
            fg_color=("gray86", "gray17"),
        )
        self.twistedRealmLabel.place(x=10, y=370)
        self.twistedRealmCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Handle Lab
        self.runLabLabel = customtkinter.CTkLabel(
            master=self.activityFrame, text="Run Lab", fg_color=("gray86", "gray17")
        )
        self.runLabLabel.place(x=10, y=400)
        self.runLabCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Collect Quests
        self.collectQuestsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect Daily/Weekly Quests",
            fg_color=("gray86", "gray17"),
        )
        self.collectQuestsLabel.place(x=10, y=430)
        self.collectQuestsCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.collectMerchantsLabel = customtkinter.CTkLabel(
            master=self.activityFrame,
            text="Collect Merchant Deals/Nobles",
            fg_color=("gray86", "gray17"),
        )
        self.collectMerchantsLabel.place(x=10, y=460)
        self.collectMerchantsCheckbox = customtkinter.CTkCheckBox(
            master=self.activityFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.label = customtkinter.CTkLabel(
            master=self.ArenaFrame, text="Arena:", font=("Arial", 15, "bold")
        )
        self.label.place(x=10, y=5)

        # Battle Arena
        self.battleArenaLabel = customtkinter.CTkLabel(
            master=self.ArenaFrame,
            text="Battle Arena of Heroes",
            fg_color=("gray86", "gray17"),
        )
        self.battleArenaLabel.place(x=10, y=40)
        self.battleArenaCheckbox = customtkinter.CTkCheckBox(
            master=self.ArenaFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Arena Battles
        self.arenaBattlesLabel = customtkinter.CTkLabel(
            master=self.ArenaFrame,
            text="Number of Battles",
            fg_color=("gray86", "gray17"),
        )
        self.arenaBattlesLabel.place(x=40, y=70)
        self.arenaBattlesEntry = customtkinter.CTkEntry(
            master=self.ArenaFrame, height=20, width=25
        )
        self.arenaBattlesEntry.insert("end", config.get("ARENA", "arenabattles"))
        self.arenaBattlesEntry.place(x=198, y=70)
        # Arena Opponent
        self.arenaOpponentLabel = customtkinter.CTkLabel(
            master=self.ArenaFrame, text="Which Opponent", fg_color=("gray86", "gray17")
        )
        self.arenaOpponentLabel.place(x=40, y=100)
        self.arenaOpponentEntry = customtkinter.CTkEntry(
            master=self.ArenaFrame, height=20, width=25
        )
        ...
        self.tsCollectLabel = customtkinter.CTkLabel(
            master=self.ArenaFrame,
            text="Collect Daily TS loot",
            fg_color=("gray86", "gray17"),
        )
        self.tsCollectLabel.place(x=10, y=130)
        self.tsCollectCheckbox = customtkinter.CTkCheckBox(
            master=self.ArenaFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.gladiatorCollectLabel = customtkinter.CTkLabel(
            master=self.ArenaFrame,
            text="Collect Gladiator Coins",
            fg_color=("gray86", "gray17"),
        )
        self.gladiatorCollectLabel.place(x=10, y=160)
        self.gladiatorCollectCheckbox = customtkinter.CTkCheckBox(
            master=self.ArenaFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.label = customtkinter.CTkLabel(
            master=self.eventsFrame, text="Events:", font=("Arial", 15, "bold")
        )
        self.label.place(x=10, y=5)

        # Fight of Fates
        self.fightOfFatesLabel = customtkinter.CTkLabel(
            master=self.eventsFrame,
            text="Fight of Fates",
            fg_color=("gray86", "gray17"),
        )
        self.fightOfFatesLabel.place(x=10, y=40)
        self.fightOfFatesCheckbox = customtkinter.CTkCheckBox(
            master=self.eventsFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Battle of Blood
        self.battleOfBloodLabel = customtkinter.CTkLabel(
            master=self.eventsFrame,
            text="Battle of Blood",
            fg_color=("gray86", "gray17"),
        )
        self.battleOfBloodLabel.place(x=10, y=70)
        self.battleOfBloodCheckbox = customtkinter.CTkCheckBox(
            master=self.eventsFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Circus Tour
        self.circusTourLabel = customtkinter.CTkLabel(
            master=self.eventsFrame, text="Circus Tour", fg_color=("gray86", "gray17")
        )
        self.circusTourLabel.place(x=10, y=100)
        self.circusTourCheckbox = customtkinter.CTkCheckBox(
            master=self.eventsFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Heroes of Esperia
        self.heroesOfEsperiaLabel = customtkinter.CTkLabel(
            master=self.eventsFrame,
            text="Heoes of Esperia",
            fg_color=("gray86", "gray17"),
        )
        self.heroesOfEsperiaLabel.place(x=10, y=130)
        self.heroesOfEsperiaCheckbox = customtkinter.CTkCheckBox(
            master=self.eventsFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.label = customtkinter.CTkLabel(
            master=self.BountiesFrame, text="Bounties:", font=("Arial", 15, "bold")
        )
        self.label.place(x=10, y=5)

        # Enable bounties
        self.dispatchSoloBountiesLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Enable solo bounties",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchSoloBountiesLabel.place(x=10, y=40)
        self.dispatchSoloBountiesCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.dispatchTeamBountiesLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Enable team bounties",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchTeamBountiesLabel.place(x=10, y=70)
        self.dispatchTeamBountiesCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.dispatchDustLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Dispatch Dust",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchDustLabel.place(x=10, y=100)
        self.dispatchDustCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.dispatchDiamondsLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Dispatch Diamonds",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchDiamondsLabel.place(x=10, y=130)
        self.dispatchDiamondsCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.dispatchShardsLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Dispatch Shards",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchShardsLabel.place(x=10, y=160)
        self.dispatchShardsCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.dispatchJuiceLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Dispatch Juice",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchJuiceLabel.place(x=10, y=190)
        self.dispatchJuiceCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Refreshes
        self.refreshLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Number of Refreshes",
            fg_color=("gray86", "gray17"),
        )
        self.refreshLabel.place(x=10, y=220)
        self.refreshEntry = customtkinter.CTkEntry(
            master=self.BountiesFrame, height=20, width=25
        )
        self.refreshEntry.insert("end", config.get("BOUNTIES", "refreshes"))
        self.refreshEntry.place(x=200, y=220)
        # Remaining
        self.remainingLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="# Remaining to Dispatch All",
            fg_color=("gray86", "gray17"),
        )
        self.remainingLabel.place(x=10, y=250)
        self.remainingEntry = customtkinter.CTkEntry(
            master=self.BountiesFrame, height=20, width=25
        )
        ...
        self.dispatchEventBountiesLabel = customtkinter.CTkLabel(
            master=self.BountiesFrame,
            text="Enable event bounties",
            fg_color=("gray86", "gray17"),
        )
        self.dispatchEventBountiesLabel.place(x=10, y=280)
        self.dispatchEventBountiesCheckbox = customtkinter.CTkCheckBox(
            master=self.BountiesFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        self.label = customtkinter.CTkLabel(
            master=self.MiscFrame, text="Misc:", font=("Arial", 15, "bold")
        )
        self.label.place(x=10, y=5)

        # Use Bag Comsumables
        self.useBagConsumablesLabel = customtkinter.CTkLabel(
            master=self.MiscFrame,
            text="Use Bag Consumables",
            fg_color=("gray86", "gray17"),
        )
        self.useBagConsumablesLabel.place(x=10, y=40)
        self.useBagConsumablesCheckbox = customtkinter.CTkCheckBox(
            master=self.MiscFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Delayed start
        self.delayedStartLabel = customtkinter.CTkLabel(
            master=self.MiscFrame,
            text="Delay start by x minutes",
            fg_color=("gray86", "gray17"),
        )
        self.delayedStartLabel.place(x=10, y=70)
        self.delayedStartEntry = customtkinter.CTkEntry(
            master=self.MiscFrame, height=20, width=25
        )
        self.delayedStartEntry.insert("end", config.get("DAILIES", "delayedstart"))
        self.delayedStartEntry.place(x=200, y=70)

        # Hibernate system
        self.hibernateLabel = customtkinter.CTkLabel(
            master=self.MiscFrame,
            text="Hibernate system when done",
            fg_color=("gray86", "gray17"),
        )
        self.hibernateLabel.place(x=10, y=100)
        self.hibernateCheckbox = customtkinter.CTkCheckBox(
            master=self.MiscFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Save button
        self.activitySaveButton = customtkinter.CTkButton(
            master=self,
            text="Save",
            fg_color=["#3B8ED0", "#1F6AA5"],
            width=120,
            command=self.activitySave,
        )
        ...
        activityBoxes = [
            ...
            "useBagConsumables",
            "heroesOfEsperia",
            "dispatchSoloBounties",
            "dispatchTeamBounties",
            "hibernate",
        ...
        for activity in activityBoxes:
            if activity[0:8] == "dispatch":
                ...
                activity == "battleArena"
                or activity == "tsCollect"
                or activity == "gladiatorCollect"
            ):
                if config.getboolean("ARENA", activity):
                    self.__getattribute__(activity + "Checkbox").select()
            elif (
                activity == "fightOfFates"
                or activity == "battleOfBlood"
                or activity == "circusTour"
                or activity == "heroesOfEsperia"
            ):
...
class shopWindow(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.label = customtkinter.CTkLabel(
            master=self.shopGoldFrame,
            text="Gold Purchases:",
            font=("Arial", 15, "bold"),
        )
        self.label.place(x=10, y=5)
        # Dim Frame
        self.shopDiamondFrame = customtkinter.CTkFrame(
            master=self, width=200, height=380
        )
        self.shopDiamondFrame.place(x=220, y=10)
        self.label = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Diamond Purchases:",
            font=("Arial", 15, "bold"),
        )
        self.label.place(x=10, y=5)
        ...
        # Shards Gold
        self.shards_goldLabel = customtkinter.CTkLabel(
            master=self.shopGoldFrame, text="Shards", fg_color=("gray86", "gray17")
        )
        self.shards_goldLabel.place(x=10, y=40)
        self.shards_goldCheckbox = customtkinter.CTkCheckBox(
            master=self.shopGoldFrame,
            text=None,
            onvalue=True,
            offvalue=False,
            command=self.shopUpdate,
        )
        self.shards_goldCheckbox.place(x=165, y=40)
        # Dust Gold
        self.dust_goldLabel = customtkinter.CTkLabel(
            master=self.shopGoldFrame, text="Dust", fg_color=("gray86", "gray17")
        )
        self.dust_goldLabel.place(x=10, y=70)
        self.dust_goldCheckbox = customtkinter.CTkCheckBox(
            master=self.shopGoldFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Silver Emblems
        self.silver_emblemLabel = customtkinter.CTkLabel(
            master=self.shopGoldFrame,
            text="Silver Emblems",
            fg_color=("gray86", "gray17"),
        )
        self.silver_emblemLabel.place(x=10, y=100)
        self.silver_emblemCheckbox = customtkinter.CTkCheckBox(
            master=self.shopGoldFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Gold Emblems
        self.gold_emblemLabel = customtkinter.CTkLabel(
            master=self.shopGoldFrame,
            text="Gold Emblems",
            fg_color=("gray86", "gray17"),
        )
        self.gold_emblemLabel.place(x=10, y=130)
        self.gold_emblemCheckbox = customtkinter.CTkCheckBox(
            master=self.shopGoldFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # PoE Gold
        self.poeLabel = customtkinter.CTkLabel(
            master=self.shopGoldFrame, text="POE", fg_color=("gray86", "gray17")
        )
        self.poeLabel.place(x=10, y=160)
        self.poeCheckbox = customtkinter.CTkCheckBox(
            master=self.shopGoldFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Timegazer
        self.timegazerLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Timegazer Card",
            fg_color=("gray86", "gray17"),
        )
        self.timegazerLabel.place(x=10, y=40)
        self.timegazerCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Arcane Staffs
        self.arcanestaffsLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Arcane Staffs",
            fg_color=("gray86", "gray17"),
        )
        self.arcanestaffsLabel.place(x=10, y=70)
        self.arcanestaffsCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Bait
        self.baitsLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame, text="Baits", fg_color=("gray86", "gray17")
        )
        self.baitsLabel.place(x=10, y=100)
        self.baitsCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Cores
        self.coresLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame, text="Cores", fg_color=("gray86", "gray17")
        )
        self.coresLabel.place(x=10, y=130)
        self.coresCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Dust Diamonds
        self.dust_diamondLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame, text="Dust", fg_color=("gray86", "gray17")
        )
        self.dust_diamondLabel.place(x=10, y=160)
        self.dust_diamondCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Elite Soulstone
        self.elite_soulstoneLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Elite Soulstone",
            fg_color=("gray86", "gray17"),
        )
        self.elite_soulstoneLabel.place(x=10, y=190)
        self.elite_soulstoneCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Superb Soulstone
        self.superb_soulstoneLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Superb Soulstone",
            fg_color=("gray86", "gray17"),
        )
        self.superb_soulstoneLabel.place(x=10, y=220)
        self.superb_soulstoneCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Quick
        self.quickLabel = customtkinter.CTkLabel(
            master=self.shopDiamondFrame,
            text="Ignore above, do quickbuy",
            fg_color=("gray86", "gray17"),
        )
        self.quickLabel.place(x=10, y=350)
        self.quickCheckbox = customtkinter.CTkCheckBox(
            master=self.shopDiamondFrame,
            text=None,
            onvalue=True,
            offvalue=False,
        ...
        # Save button
        self.shopSaveButton = customtkinter.CTkButton(
            master=self,
            text="Save",
            fg_color=["#3B8ED0", "#1F6AA5"],
            width=120,
            command=self.shopSave,
        )
...
class advancedWindow(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        # Port Entry
        self.portLabel = customtkinter.CTkLabel(
            master=self.advancedFrame, text="Port:", fg_color=("gray86", "gray17")
        )
        self.portLabel.place(x=10, y=40)
        self.portEntry = customtkinter.CTkEntry(
            master=self.advancedFrame, height=25, width=45
        )
        self.portEntry.insert("end", config.get("ADVANCED", "port"))
        self.portEntry.place(x=275, y=40)

        # Loading.. Entry
        self.delayLabel = customtkinter.CTkLabel(
            master=self.advancedFrame,
            text="Delay multiplier:",
            fg_color=("gray86", "gray17"),
        )
        self.delayLabel.place(x=10, y=70)
        self.delayEntry = customtkinter.CTkEntry(
            master=self.advancedFrame, height=25, width=45
        )
        ...
        self.victoryCheckLabel = customtkinter.CTkLabel(
            master=self.advancedFrame,
            text="Victory Check Frequency:",
            fg_color=("gray86", "gray17"),
        )
        self.victoryCheckLabel.place(x=10, y=100)
        self.victoryCheckEntry = customtkinter.CTkEntry(
            master=self.advancedFrame, height=25, width=45
        )
        ...
        self.supressLabel = customtkinter.CTkLabel(
            master=self.advancedFrame,
            text="Suppress victory check spam?",
            fg_color=("gray86", "gray17"),
        )
        self.supressLabel.place(x=10, y=130)
        self.supressCheckbox = customtkinter.CTkCheckBox(
            master=self.advancedFrame, text=None, onvalue=True, offvalue=False
        )
        self.supressCheckbox.place(x=295, y=130)

        # Use popular formations
        self.popularFormationsLabel = customtkinter.CTkLabel(
            master=self.advancedFrame,
            text="Use popular formations",
            fg_color=("gray86", "gray17"),
        )
        self.popularFormationsLabel.place(x=10, y=160)
        self.popularFormationsCheckbox = customtkinter.CTkCheckBox(
            master=self.advancedFrame, text=None, onvalue=True, offvalue=False
        )
        self.popularFormationsCheckbox.place(x=295, y=160)

        # Debug Mode
        self.debugLabel = customtkinter.CTkLabel(
            master=self.advancedFrame, text="Debug Mode", fg_color=("gray86", "gray17")
        )
        self.debugLabel.place(x=10, y=190)
        self.debugCheckbox = customtkinter.CTkCheckBox(
            master=self.advancedFrame, text=None, onvalue=True, offvalue=False
        )
        self.debugCheckbox.place(x=295, y=190)

        # Emulator path
        self.emulatorPathLabel = customtkinter.CTkLabel(
            master=self.advancedFrame,
            text="Emulator executable path (optional):",
            fg_color=("gray86", "gray17"),
        )
        self.emulatorPathLabel.place(x=10, y=220)
        self.emulatorPathEntry = customtkinter.CTkEntry(
            master=self.advancedFrame, height=25, width=100
        )
        self.emulatorPathEntry.insert("end", config.get("ADVANCED", "emulatorpath"))
        self.emulatorPathEntry.place(x=220, y=220)

        # Save button
        self.advanceSaveButton = customtkinter.CTkButton(
            master=self,
            text="Save",
            fg_color=["#3B8ED0", "#1F6AA5"],
            width=120,
            command=self.advancedSaveButton,
        )
    ...
    def advancedSave(self):
        ...
        if self.popularFormationsCheckbox.get() != config.getboolean(
            "ADVANCED", "popularformations"
        ):
...
def pause_all_thread():
    ...
    app.pauseButton.configure(
        text="▶️", command=resume_all_thread, fg_color="green"
...
def dailies():
    ...
    count_api = (
        "https://api.api-ninjas.com/v1/counter?id=AutoAFK-" + version + "-run&hit=true"
    )
    hit = requests.get(
        count_api, headers={"X-Api-Key": "Nc9+gX2u0w/F5smHLYOrdg==3Mh0tPQWWux2OZsA"}
    )
    ...
    while app.dailies_thread_running or args["dailies"]:
        ...
        if config.getboolean("DAILIES", "attemptcampaign"):
            ...
            if pauseOrStopEventCheck(app.dailies_pause_event, app.dailies_stop_event):
                break  # Exit the loop if stop event is set
        if (
            config.getboolean("BOUNTIES", "dispatchsolobounties")
